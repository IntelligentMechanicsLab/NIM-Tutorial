...
from core.time_dependent_NIM import NIM_time_dependent_data
...
NPu_inter = 5
x_inter_1 = -0.5 * np.ones((NPu_inter,1))
t_inter_1 = T*random.uniform(keys[0], (NPu_inter, 1))

x_inter_2 =  np.zeros((NPu_inter,1))
t_inter_2 = T*random.uniform(keys[0], (NPu_inter, 1))

x_inter_3 = 0.5 * np.ones((NPu_inter,1))
t_inter_3 = T*random.uniform(keys[0], (NPu_inter, 1))

x_data = np.concatenate([x_inter_1, x_inter_2, x_inter_3])
t_data = np.concatenate([t_inter_1, t_inter_2, t_inter_3])
xt_data = np.hstack((x_data,t_data))
c_data = c_ext_fun(xt_data, epsilon, V).reshape(-1,1)
...
path_f='Results' + '/' + 'ADE_inverse/'  + str(Nx) + '_' + str(Xi_rows.shape[0]) + '/'
...
PHI_data, _, _, index_data = calculate_shape_function(x_data.reshape(-1,1))
...
layers = [m, 30, 30, 30, Xi.shape[0]]


model = NIM_time_dependent_data(layers)
...
params = model.train(batch_size, [0,1], nIter=100000)
...
np.save(path_f+'adv_loss_data_c.npy', model.loss_data_c_log)
np.save(path_f+'adv_coe.npy', model.coe_log)
###################################################
...
coe_log = np.load(path_f+'adv_coe.npy')
...
plt.savefig(path_f+'coe_identification.png')
